...
public class CarManagerImpl implements ICarManager {
	...
	// 익명클래스 문법
	// - 인터페이스를 생성자처럼쓰고 그 뒤에 {} 안에 메소드를 오버라이딩.
	// - 추상클래스도 생성자+{추상메서드 오버라이딩}
	
	private static CarManagerImpl instance = new CarManagerImpl() ;// 익명 클래스 문법
		// {} 중괄호 안에다가 추상메서드를 모두 구현하는 임시 클래스 본문을 쓸 수 있음.
		// 일회용 클래스.
		
	
//	private static ICarManager instance2 = new ICarManager(); // 인터페이스로는 객체 생성 x
	//private static ICarManager instance2 = new CarManagerImpl(); // 다형성

	// 외부에서 객체 생성을 하지 못하도록 생성자를 싱글톤 패턴 으로 선언
